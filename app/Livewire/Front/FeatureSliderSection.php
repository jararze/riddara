...
class FeatureSliderSection extends Component
{
    ...
    public function mount($vehicle = [], $section = 'potente_dinamico', $featureData = [])
    {
        ...
        // Obtener configuración de la sección específica del vehículo
        $sectionConfig = $vehicleConfigs[$section] ?? null;

        // Si no existe configuración específica del vehículo, usar el default
        if ($sectionConfig === null) {
            // Verificar si existe en los defaults
            if (isset($this->defaultFeatureData[$section])) {
                $this->featureData = $this->defaultFeatureData[$section];
            ...
            // Existe configuración específica, hacer merge con defaults
            $this->sectionAvailable = true;

            // Empezar con los defaults de la sección
            $defaultSection = $this->defaultFeatureData[$section] ?? [];

            // Hacer merge profundo
            $this->featureData = array_replace_recursive($defaultSection, $sectionConfig);
        }

        // Deshabilitar autoplay por defecto
